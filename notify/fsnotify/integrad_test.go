...
func newWatcher(t *testing.T) notify.Watcher {
...
func addWatch(t *testing.T, watcher notify.Watcher, dir string) {
...
func TestFsnotifyMultipleOperations(t *testing.T) {
	...
	eventstream := watcher.EventChannel()
	...
	go func() {
		for event := range eventstream {
			...
			if event.Name == filepath.Clean(testDir) || event.Name == filepath.Clean(testFile) {
				...
				if event.Op&notify.Write == notify.Write {
					modifyReceived.increment()
				}
				if event.Op&notify.Create == notify.Create {
					createReceived.increment()
				}
				if event.Op&notify.Rename == notify.Rename {
	...
	go func() {
		for event := range eventstream {
			...
			if event.Name == filepath.Clean(testDir) || event.Name == filepath.Clean(testFile) {
				...
				if event.Op&notify.Chmod == notify.Chmod {
...
func TestFsnotifyFakeSymlink(t *testing.T) {
	...
	// Receive errors on the error channel on a separate goroutine
	go func() {
		for errors := range watcher.ErrorChannel() {
	...
	go func() {
		for ev := range watcher.EventChannel() {
