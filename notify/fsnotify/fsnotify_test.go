...
func TestEventStringWithValue(t *testing.T) {
	for opMask, expectedString := range map[notify.Op]string{
		notify.Chmod | notify.Create: `"/usr/someFile": CREATE|CHMOD`,
		notify.Rename:         `"/usr/someFile": RENAME`,
		notify.Remove:         `"/usr/someFile": REMOVE`,
		notify.Write | notify.Chmod:  `"/usr/someFile": WRITE|CHMOD`,
	} {
		event := notify.Event{Name: "/usr/someFile", Op: opMask}
...
func TestEventOpStringWithValue(t *testing.T) {
	...
	event := notify.Event{Name: "someFile", Op: notify.Write | notify.Chmod}
...
func TestEventOpStringWithNoValue(t *testing.T) {
	expectedOpString := ""
	event := notify.Event{Name: "testFile", Op: 0}
	if event.Op.String() != expectedOpString {
		t.Fatalf("Expected %s, got: %v", expectedOpString, event.Op.String())
	}
}
