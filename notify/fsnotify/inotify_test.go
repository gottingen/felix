...
func isWatcherReallyClosed(t *testing.T, w notify.Watcher) {
	select {
	case err, ok := <-w.ErrorChannel():
		if ok {
			t.Fatalf("w.ErrorChannel is not closed; readEvents is still alive after closing (error: %v)", err)
		}
	default:
		t.Fatalf("w.ErrorChannel would have blocked; readEvents is still alive!")
	}

	select {
	case _, ok := <-w.EventChannel():
		if ok {
			t.Fatalf("w.EventChannel() is not closed; readEvents is still alive after closing")
		}
	default:
		t.Fatalf("w.EventChannel() would have blocked; readEvents is still alive!")
	}
}
...
func TestInotifyStress(t *testing.T) {
	...
	for !finished {
		...
		case evt := <-w.EventChannel():
			if !strings.HasPrefix(evt.Name, testFilePrefix) {
				t.Fatalf("Got an event for an unknown file: %s", evt.Name)
			}
			if evt.Op == notify.Create {
				creates++
			}
			if evt.Op == notify.Remove {
				removes++
			}
		}
	}
...
func TestInotifyRemoveTwice(t *testing.T) {
	...
	defer lw.mu.Unlock()
	if len(lw.watches) != 0 {
		t.Fatalf("Expected watches len is 0, but got: %d, %v", len(lw.watches), lw.watches)
	}
	if len(lw.paths) != 0 {
		t.Fatalf("Expected paths len is 0, but got: %d, %v", len(lw.paths), lw.paths)
	}
}
...
func TestInotifyInnerMapLength(t *testing.T) {
	...
	_ = <-w.EventChannel()                      // consume Remove event
...
func TestInotifyOverflow(t *testing.T) {
	...
	for overflows == 0 && creates < numDirs*numFiles {
		...
		case err := <-errChan:
			t.Fatalf("Got an error from file creator goroutine: %v", err)
		case err := <-w.ErrorChannel():
			if err == notify.ErrEventOverflow {
				overflows++
			} else {
				t.Fatalf("Got an error from watcher: %v", err)
			}
